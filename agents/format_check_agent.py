...
def check_incomplete_content(text: str, source: str = "") -> list[str]:
    ...
    """
    try:
        from utils.paper_quality_guardrail import (
...
def run_format_check(paper_only: bool = False, json_only: bool = False) -> dict:
    ...
    if not json_only:
        # LaTeX paper (no markdown)
        tex_path = os.path.join(OUTPUT_DIR, "paper", "paper.tex")
        draft = _read_file(tex_path)
        if draft:
            issues.extend(check_garbled_text(draft, "paper.tex"))
            issues.extend(check_incomplete_content(draft, "paper.tex"))
        else:
...
def apply_format_fixes() -> bool:
    """
    Apply standard format fixes to paper outputs (LaTeX).
    ...
    if not draft:
        return False
    fixed = False
    # LaTeX table fixes: ensure SPD Viol / EOD Viol in header
    if "\\\\midrule" in draft and "SPD Viol" not in draft and "Model & Acc" in draft:
        # Tables are generated by auditing_agent; fix would require section edit
        pass  # LaTeX tables from _latex_metrics_table already have correct headers
    return fixed
