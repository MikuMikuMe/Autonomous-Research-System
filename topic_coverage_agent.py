...
Runs after the Auditing Agent produces paper_draft.md.
"""
...
PAPER_DRAFT = os.path.join(OUTPUT_DIR, "paper_draft.md")
...
def _load_paper_draft() -> str:
    """Load paper_draft.md or paper.tex as fallback."""
    if os.path.exists(PAPER_DRAFT):
        with open(PAPER_DRAFT, encoding="utf-8") as f:
            return f.read()
...
def run_topic_coverage() -> dict:
    """
    Extract topics from source PDFs, check coverage in paper_draft.md.
    Returns report with covered/missing topics per PDF.
    """
    try:
        from pdf_source_extractor import extract_topics_from_pdfs
    except ImportError:
        return {
    ...
    if not paper_text or len(paper_text.strip()) < 500:
        return {
            "error": "paper_draft.md or paper.tex not found or too short. Run auditing_agent first.",
            "timestamp": datetime.now().isoformat(),
        }
    ...
    report = {
        "timestamp": datetime.now().isoformat(),
        "paper_source": PAPER_DRAFT if os.path.exists(PAPER_DRAFT) else "paper/paper.tex",
        "paper_length": len(paper_text),
