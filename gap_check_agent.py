...
CHECKLIST_PATH = os.path.join(SCRIPT_DIR, "docs", "RESEARCH_CHECKLIST.md")
...
def _write_gaps_section(report: dict) -> None:
    """Generate a paper section for each identified gap. Saved to paper_sections/07_gaps.md."""
    ...
    lines = [
        ...
        "",
        "The following topics from our reference literature (how_biases_are_introduced.pdf) "
    ...
    for i, g in enumerate(report["gaps"], 1):
        ...
        lines.extend([
            ...
            "",
            _gap_section_content(topic, keywords),
            "",
        ])
    ...
    path = os.path.join(sections_dir, "07_gaps.md")
...
def _recompile_paper_with_gaps() -> None:
    """Recompile paper_draft.md and regenerate PDF to include the new gaps section."""
    try:
        from auditing_agent import compile_draft
        compile_draft()
        print("  Recompiled paper_draft.md with gap sections.")
        # Regenerate PDF so it matches the updated paper
        try:
            from latex_generator import compile_latex
