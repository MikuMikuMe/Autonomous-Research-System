...
Wraps subprocess.run with security hardening for executing LLM-generated code:
  ...
  - Import whitelist / blocklist for generated Python code
  - Restricted environment variables (stripped API keys)
  - Temporary directory isolation
  - Optional Docker container isolation

Usage:
    from utils.sandbox import run_sandboxed, execute_code, SandboxConfig
    result = run_sandboxed(code_string, timeout=30, max_memory_mb=512)
    result = execute_code(code_string, config=SandboxConfig.from_config())
"""
...
_SENSITIVE_ENV_KEYS = {
    ...
    "LANGCHAIN_API_KEY", "TAVILY_API_KEY", "TELEGRAM_BOT_TOKEN",
}
...
class SandboxConfig:
    ...
    blocked_builtins: list[str] = field(default_factory=lambda: [
        "exec", "eval", "breakpoint", "exit", "quit",
    ])
...
def _make_safe_env(extra_env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    for key, value in os.environ.items():
        if key.upper() in _SENSITIVE_ENV_KEYS:
            continue
        lower_key = key.lower()
        if any(s in lower_key for s in ("secret", "token", "password", "api_key", "apikey")):
            continue
        env[key] = value
...
def _check_imports(code: str, config: SandboxConfig) -> list[str]:
    ...
    for match in import_pattern.finditer(code):
        ...
        for b in config.blocked_imports:
            if full_module.startswith(b) and full_module not in blocked:
...
def _make_prelude(max_memory_mb: int | None = None) -> str:
    """Generate Python prelude that enforces resource limits from inside the sandbox."""
    lines = [
        "import sys",
...
def run_sandboxed(
    ...
    *,
    ...
    Backward-compatible API. For enhanced security, use execute_code() instead.
...
def execute_code(
    ...
    *,
    ...
    """Execute Python code in a hardened sandboxed environment.

    Enhanced API with import blocking, Docker support, and config-driven settings.
