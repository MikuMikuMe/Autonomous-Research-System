...
def collect_pdf_citations() -> list[dict]:
    ...
    try:
        from utils.pdf_source_extractor import get_all_citations
...
def format_references_latex(
    static_refs: list[str],
    dynamic_citations: list[dict],
    pdf_citations: list[dict] | None = None,
) -> str:
    """LaTeX bibliography commands. For LaTeX-only pipeline, references come from references.bib.
    This returns the \\bibliographystyle and \\bibliography commands."""
    return r"""
